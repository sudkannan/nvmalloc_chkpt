...
void bitonicMerge__cuda (unsigned int n, keytype* A, int isrestart)
{
  init_param();
...
//  keytype* tmp_gpu = createKeysOnGPU (n);
  ...
  unsigned int shift = 0;
   keytype* A_gpu;
  	...
  	if(!isrestart) {
	  /* Copy A to A_gpu, which resides on the GPU */
	  A_gpu = createKeysOnGPU (n);
	  copyKeysToGPU (n, A_gpu, A);

	  for (unsigned int t_ = 1; t_ != n_half; t_ <<= 1)
		  ++ shift;
	}else {	
		 fprintf(stdout,"in restart mode... \n");
		 //load already present keys
		 A_gpu = loadKeysOnGPU (n);
		 //update the offset and shift fields	
		 shift = *g_shift;
		 offset = *g_offset;
		 fprintf(stdout, "shift: %d, offset %d \n",shift,offset);
	}

  {
	fprintf(stdout,"launching kernel here \n");
    if (BLOCKSIZE == 512)
    {
 	 ...
 	 if(offset == 1024) {
	 	cudaDeviceSynchronize();
	    chk_cudaCommit(NULL, offset);
   	 }

	 //inject failure 
	 //if restart skil failure injection
 	 if(!isrestart && offset < 1024) {
		fprintf(stdout, "simulating failure and killing self \n");
		exit(-1);
   	 }
#endif
...
createKeysOnGPU (unsigned int n)
{
  keytype* A_gpu = NULL;
  if (n) {
    chk_cudaMalloc ((void **)&A_gpu, n * sizeof (keytype), "A_gpu"); CHECK_GPU ("Out of memory?");

     g_offset = (unsigned int *)chk_cpuMalloc(sizeof(unsigned int), "g_offset");
     g_shift =  (unsigned int *) chk_cpuMalloc(sizeof(unsigned int), "g_shift");		
...
keytype *
loadKeysOnGPU (unsigned int n)
{
  keytype* A_gpu = NULL;
  if (n) {

     chk_cudaMalloc_restart ((void **)&A_gpu, n * sizeof (keytype), "A_gpu"); CHECK_GPU ("Out of memory?");

     g_offset = (unsigned int *)chk_cpuMalloc_restart(sizeof(unsigned int), "g_offset");
     g_shift =  (unsigned int *) chk_cpuMalloc_restart(sizeof(unsigned int), "g_shift");		
