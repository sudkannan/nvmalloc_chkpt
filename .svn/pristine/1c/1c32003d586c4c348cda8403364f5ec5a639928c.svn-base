...
#endif
__global__ void gpuRL_listrank1 (int n, int* Rank_gpu, int* Next_gpu, int * Rank_next_gpu, int *Next_next_gpu) {
  ...
  if (i < n)	
  {
       ...
       if (Next_gpu[i] != NIL){
         Rank_next_gpu[i] = Rank_gpu[Next_gpu[i]];
         Next_next_gpu[i] = Next_gpu[Next_gpu[i]];
       }
	/* __syncthreads();	
     if (Next_gpu[i] != NIL){
        Rank_gpu[i] += Rank_next_gpu[i];
        Next_gpu[i] = Next_next_gpu[i];
      }*/
	}
}

__global__ void gpuRL_listrank2 (int n, int* Rank_gpu, int* Next_gpu, int * Rank_next_gpu, int *Next_next_gpu) {
...
void cpu_calllistrank (int TPB, int NBLOCKS, int n, int* Rank_gpu, int* Next_gpu, int * Rank_next_gpu, int *Next_next_gpu) {

	for (int j = 1; j < n; j = j*2) {
			gpuRL_listrank1<<<NBLOCKS, TPB>>> (n, Rank_gpu, Next_gpu, Rank_next_gpu, Next_next_gpu);	
			cudaDeviceSynchronize ();
			gpuRL_listrank2<<<NBLOCKS, TPB>>> (n, Rank_gpu, Next_gpu, Rank_next_gpu, Next_next_gpu);	
			cudaDeviceSynchronize ();

      }
}

__global__ void gpu_ranklist_Init (int n, int* Rank_gpu, int* Next_gpu, int * Rank_next_gpu, int *Next_next_gpu) {

  int i = blockIdx.x * blockDim.x + threadIdx.x;

  if (i < n) {
		Rank_gpu[i] = (Next_gpu[i] == NIL) ? 0 : 1;
		Rank_next_gpu[i] = (Next_gpu[i] == NIL) ? 0 : 1;
	}
}



void create_gpu_copy (int n, int* Rank, const int *Next, int**Rank_gpu, int **Next_gpu, int **Rank_next_gpu, int **Next_next_gpu) {

  CUDA_CHECK_ERROR(cudaMalloc (Rank_gpu, n * sizeof (int)));
  CUDA_CHECK_ERROR( cudaMalloc (Rank_next_gpu, n * sizeof (int)));
  CUDA_CHECK_ERROR( cudaMalloc (Next_gpu, n * sizeof (int)));
  CUDA_CHECK_ERROR(cudaMalloc (Next_next_gpu, n * sizeof (int)));

  CUDA_CHECK_ERROR(cudaMemcpy (*Rank_gpu, Rank, n * sizeof (int), cudaMemcpyHostToDevice));
  CUDA_CHECK_ERROR(cudaMemcpy (*Next_gpu, Next, n * sizeof (int),cudaMemcpyHostToDevice));
  CUDA_CHECK_ERROR(cudaMemcpy (*Rank_next_gpu, Rank, n * sizeof (int), cudaMemcpyHostToDevice));
  CUDA_CHECK_ERROR(cudaMemcpy (*Next_next_gpu, Next, n * sizeof (int),cudaMemcpyHostToDevice));

}

void create_cpu_copy (int n, int* Rank, int**Rank_gpu) {

  cudaMemcpy (Rank, *Rank_gpu, n * sizeof (int), cudaMemcpyDeviceToHost);

}

void free_gpu_mem( int**Rank_gpu, int **Next_gpu, int **Rank_next_gpu, int **Next_next_gpu) {

  cudaFree(*Rank_gpu);
  cudaFree(*Next_gpu);
  cudaFree(*Next_next_gpu);
  cudaFree(*Rank_next_gpu);
}


void rankList__gpu (int n, int* Rank, const int* Next, int**Rank_gpu, int **Next_gpu, int **Rank_next_gpu, int **Next_next_gpu) {


  const int TPB = 512;  // threads per block
  const int NBLOCKS = (n + 512 - 1) / 512;  // ceil (n/128)

  gpu_ranklist_Init<<<NBLOCKS, TPB>>> (n, *Rank_gpu, *Next_gpu, *Rank_next_gpu, *Next_next_gpu);

  cudaDeviceSynchronize ();

   cpu_calllistrank(TPB, NBLOCKS, n, *Rank_gpu, *Next_gpu, *Rank_next_gpu, *Next_next_gpu);
}
...
main (int argc, char* argv[])
{
  ...
  for (int trial = 0; trial < NTRIALS; ++trial) {
	...
	int *Rank_gpu, *Next_gpu, *Rank_next_gpu, *Next_next_gpu;
	create_gpu_copy (N, Rank_par, Next, &Rank_gpu, &Next_gpu, &Rank_next_gpu, &Next_next_gpu);

    stopwatch_start (timer);

    //rankLis__par (N, Rank_par, Next);
	//rankList__gpu (N, Rank_par, Next);

	rankList__gpu(N, Rank_par, Next, &Rank_gpu, &Next_gpu, &Rank_next_gpu, &Next_next_gpu);

    long double t_par = stopwatch_stop (timer); /* seconds */
    long double bw_par = estimateBandwidth (N, t_par) * 1e-9; /* GB/s */

	create_cpu_copy (N, Rank_par, &Rank_gpu);
	free_gpu_mem(&Rank_gpu,&Next_gpu, &Rank_next_gpu, &Next_next_gpu);
