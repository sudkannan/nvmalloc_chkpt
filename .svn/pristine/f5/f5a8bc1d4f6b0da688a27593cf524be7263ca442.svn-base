...
int compare_data(char *src, char *dest, size_t size){
...
static char* LoadDirFile(size_t *size, char *filename, char *read_dir) {
	...
	nvptr = (char *)nvread_len((char *)filename, 0, &chunksz); 
	fprintf(stdout, "nvptr %lu \n", (unsigned long)nvptr);
    assert(nvptr);      
    assert(chunksz == fsize);
	assert(compare_data(nvptr, input, fsize) == 0);
...
int main(int argc, char *argv[])
{
	...
	if(atoi(argv[3]) == 1){
	    ...
	    char *nvm_out_ptr = (char *)nvread_len(argv[1], 0, &nvfilesz); 
   	    assert(nvm_out_ptr);      
	    assert(nvfilesz ==  st.st_size);
	
	   /*for ( int i=0; i <  size; i++) {
		fprintf(stdout,"%c", (char)nvm_out_ptr[i]);
	   }*/
	}
	exit(0);
}
