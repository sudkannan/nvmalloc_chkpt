...
gmr_t *gmr_create(int local_size, void **base_ptrs, ARMCI_Group *group) {
  ...
  mreg = (gmr_t *)malloc(sizeof(gmr_t));
  ARMCII_Assert(mreg != NULL);

  mreg->slices = (gmr_slice_t *)malloc(sizeof(gmr_slice_t)*world_nproc);
  ARMCII_Assert(mreg->slices != NULL);
  alloc_slices = (gmr_slice_t *) malloc(sizeof(gmr_slice_t)*alloc_nproc);
...
gmr_t *gmr_lookup(void *ptr, int proc) {
  ...
  while (mreg != NULL) {
    ...
    if (proc < mreg->nslices) {
      const uint8_t *base = (const uint8_t *)mreg->slices[proc].base;
